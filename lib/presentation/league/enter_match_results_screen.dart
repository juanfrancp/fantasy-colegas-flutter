...
class _EnterMatchResultsScreenState extends State<EnterMatchResultsScreen> {
  ...
  void initState() {
    ...
    // Inicializar controladores de jugadores
  ...
  void _initializeControllers(List<Player> players) {
    for (var player in players) {
      ...
      _yellowCardControllers[player.id] = TextEditingController(text: '0');
      _redCardControllers[player.id] = TextEditingController(text: '0');
      _savesControllers[player.id] = TextEditingController(text: '0');
      _concededControllers[player.id] = TextEditingController(text: '0');
      _cleanSheetValues[player.id] = false;
    }
  }
              ...
              child: Column(
                children: [
                  ...
                  const Text("ESTADÍSTICAS INDIVIDUALES", 
                    style: TextStyle(color: AppColors.secondaryAccent, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const Text("-", style: TextStyle(color: AppColors.lightSurface, fontSize: 40, fontWeight: FontWeight.bold)),
      ...
      children: [
        Container(
          ...
          child: Text(label, 
            ...
            style: const TextStyle(color: AppColors.lightSurface, fontWeight: FontWeight.bold, fontSize: 14),
        ...
        SizedBox(
          ...
          child: TextField(
            ...
            textAlign: TextAlign.center,
            style: const TextStyle(color: AppColors.primaryAccent, fontSize: 32, fontWeight: FontWeight.bold),
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: Text(teamName, style: const TextStyle(color: AppColors.primaryAccent, fontSize: 18, fontWeight: FontWeight.bold)),
        ),
        ...players.map((player) => Card(
          ...
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(player.name, style: const TextStyle(color: AppColors.lightSurface, fontWeight: FontWeight.bold, fontSize: 16)),
                const SizedBox(height: 12),
                
                // FILA 1: Ataque y Disciplina
                Row(
                  children: [
                    _buildInput('Goles', _goalsControllers[player.id]!, color: Colors.greenAccent),
                    const SizedBox(width: 8),
                    _buildInput('Asist.', _assistsControllers[player.id]!, color: Colors.lightBlueAccent),
                    const SizedBox(width: 8),
                    _buildInput('Amaril.', _yellowCardControllers[player.id]!, color: Colors.yellowAccent),
                    const SizedBox(width: 8),
                    _buildInput('Rojas', _redCardControllers[player.id]!, color: Colors.redAccent),
                  ],
                ),
                ...
                Row(
                  children: [
                    _buildInput('Paradas', _savesControllers[player.id]!, color: Colors.orangeAccent),
                    const SizedBox(width: 8),
                    _buildInput('Encajad.', _concededControllers[player.id]!, color: Colors.deepOrangeAccent),
                    ...
                    Expanded(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                            const Text("Imbatido", style: TextStyle(color: AppColors.secondaryAccent, fontSize: 10)),
                            Switch(
  ...
  Widget _buildInput(String label, TextEditingController controller, {Color? color}) {
    return Expanded(
      ...
      child: TextField(
        ...
        keyboardType: TextInputType.number,
        textAlign: TextAlign.center,
        style: TextStyle(color: color ?? AppColors.lightSurface, fontWeight: FontWeight.bold),
        decoration: InputDecoration(
          labelText: label,
          labelStyle: TextStyle(color: AppColors.secondaryAccent.withValues(alpha: 0.8), fontSize: 11),
          floatingLabelBehavior: FloatingLabelBehavior.always,
          isDense: true,
          contentPadding: const EdgeInsets.symmetric(vertical: 8, horizontal: 4),
          border: const OutlineInputBorder(),
          enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: AppColors.secondaryAccent.withValues(alpha: 0.3))),
          focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: color ?? AppColors.primaryAccent)),
        ),
      ),
    );
  }
